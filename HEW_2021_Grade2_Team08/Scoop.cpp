...
void Icepillar::Icepillar_ColligionCheck(float delta_taime)
{
    ...
    for (int i = 0; i < GetGame()->GetEArmor().size(); ++i)
    {
        if (GetGame()->GetEArmor().at(i))
        {
            if (GetGame()->GetEArmor().at(i)->Get_Isplayer() != k_Is_player_)
            {
                if (CollisionRC_NoInd(GetGame()->GetEArmor().at(i), this) && !hitstate_)
                {
                    hitstate_ = true;
                    GetGame()->GetEArmor().at(i)->Set_Armorhit(1);
                }
    ...
    for (int i = 0; i < GetGame()->GetEGolems().size(); ++i)
    {
        if (GetGame()->GetEGolems().at(i))
        {
            if (GetGame()->GetEGolems().at(i)->Get_Isplayer() != k_Is_player_)
            {
                if (CollisionRC_NoInd(GetGame()->GetEGolems().at(i), this) && !hitstate_)
                {
                    GetGame()->GetEGolems().at(i)->Set_Golemhit(k_icepillar_power_);
                    hitstate_ = true;
                }
            }
        }
	...
	if (k_Is_player_)//プレイヤー側の弾
	{
        ...
        if (GetGame()->GetEnemy())
        {
            ...
            {
                //GetGame()->GetEnemy()->Player_SetHit(2);
