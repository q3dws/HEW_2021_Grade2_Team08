...
HRESULT InitializeMouse(HINSTANCE hInst,HWND hWindow)
{
	...
	{
		MessageBox(hWindow,L"No mouse",L"Warning",MB_OK | MB_ICONWARNING);
		return result;
	}
	// データフォーマット設定
	...
	if(FAILED(result))
	{
		MessageBox(hWindow,L"Can't setup mouse",L"Warning",MB_OK | MB_ICONWARNING);
		return result;
	}
	// 他のアプリと協調モードに設定
	...
	if(FAILED(result))
	{
		MessageBox(hWindow,L"Mouse mode error",L"Warning",MB_OK | MB_ICONWARNING);
		return result;
	}
	...
	if(FAILED(result))
	{
		MessageBox(hWindow,L"Mouse property error",L"Warning",MB_OK | MB_ICONWARNING);
		return result;	
	}
